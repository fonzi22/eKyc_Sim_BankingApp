...
class ZKPEnrollmentManager(private val context: Context) {
    ...
    fun performVerification(sessionId: String): SchnorrZKP.VerificationPayload? {
        ...
        val verificationMessage = "VERIFY:$sessionId:${System.currentTimeMillis()}"
    ...
    private fun savePrivateKey(privateKey: BigInteger) {
        prefs.edit()
            .putString("private_key", SchnorrZKP.bigIntToHex(privateKey))
            .apply()
    }
    ...
    private fun loadPrivateKey(): BigInteger? {
        val hex = prefs.getString("private_key", null) ?: return null
        return SchnorrZKP.hexToBigInt(hex)
    }
